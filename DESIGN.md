...
> 이 프로젝트의 모든 UI 텍스트와 콘텐츠는 **한국어**로 작성한다.

한국어는 라틴 알파벳과 글자 구조가 달라 별도의 한글 지원 폰트가 필요하다. ABC Ginto Nord / ggsans 자리에 아래 폰트를 대응시킨다.

| 역할 | 영문 원본 | 한국어 대체 폰트 | 특징 |
|---|---|---|---|
| Display / 헤드라인 | ABC Ginto Nord 800 | **Black Han Sans** | 굵고 임팩트 있는 블랙 웨이트, 게이밍 브랜드 톤에 부합 |
| Sub-heading / 버튼 | ABC Ginto 500 | **Gmarket Sans Bold / Medium** | 친근하고 현대적인 산세리프, 브랜드 에너지 유지 |
| Body / UI | ggsans 400 | **Noto Sans KR** 또는 **Spoqa Han Sans Neo** | 가독성 높은 범용 본문 폰트, Google Fonts에서 무료 제공 |

**적용 지침:**
- 헤드라인(`{typography.display-xl}` ~ `{typography.display-md}`)에는 **Black Han Sans**를 사용하고, 모두 대문자(ALL-CAPS) 대신 강조 어구를 굵게 처리한다 — 한글은 대·소문자 구분이 없으므로 폰트 무게와 크기로 위계를 표현한다.
- 버튼 레이블과 링크(`{typography.link-lg}` ~ `{typography.link-sm}`)에는 **Gmarket Sans Medium/Bold**를 사용한다.
- 본문 및 보조 텍스트(`{typography.body}`)에는 **Noto Sans KR 400/500**을 사용한다.
- 폰트 로드는 Google Fonts CDN(`Noto Sans KR`, `Black Han Sans`, `Gmarket Sans`)을 우선하되, 자체 호스팅이 필요할 경우 서브셋(subset) 최적화를 적용한다.
- 행 높이(line-height)는 한글 자소 구조상 라틴보다 넓게 설정한다: display 1.2, body 1.7 이상을 권장한다.
